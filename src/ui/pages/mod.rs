...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        for id in epics.keys().sorted() {
            ...
            let id_name = get_column_string(&epic.name, 32);
            let id_status = get_column_string(&epic.status.to_string(), 17);

            println!("{}|{}|{}", id_col, id_name, id_status);
        }
